...
- **Ingress/egress:** ordinarily small HTML, JavaScript, and CSS responses plus high-level status checks. `POST /_portal/media` accepts at most an 8 KiB JSON body and is the only task-specific portal request. Catalog launches navigate to separately hosted upstream applications rather than relaying their task traffic through the portal.
- **Database:** none.
- **Memory-only state:** media limiter counters clear after their configured window and sweep interval or on portal restart.
...
- **Controls:** exact Origin checks for media; strict provider/URL validation; private literal-address rejection; fixed upstream functions; media rate, body-read, upstream deadline, response-size, and concurrency limits; a fail-closed 431 at the 100-header boundary; IPv6 /64 application rate keys; fixed high-level status targets; and container ceilings. No generic server HTTP proxy, webhook receiver, or DNS lookup endpoint exists.
...
5. **Portal static assets:** bounded and cacheable. The replacement image must
   be measured after the upstream-FOSS-only cleanup.
...
- Portal media rate counters, Cobalt tunnel metadata, and Valkey limiter state are memory-only and reset on restart. Resetting counters is an availability/abuse tradeoff, not persistence failure.
...
6. one catalog-policy check proving every launchable entry maps to an approved
   independently maintained, self-hostable FOSS application;
7. short controlled concurrency within published limits, without targeting third-party services repeatedly;
8. per-service network counters and host free disk before/after those checks; and
9. reachability from the edge plus rejection from one authorized external test host.
